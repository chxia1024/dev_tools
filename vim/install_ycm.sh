...
# link libclang to the same path where ycm_core.so located in order to reuse the RPATH=$ORIGIN in ycm_core.so
/bin/ln $LLVM_INSTALL/lib/libclang.so* -s $dir/third_party/ycmd/
cd -
